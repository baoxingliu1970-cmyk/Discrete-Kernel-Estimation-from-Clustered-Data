---
title: "Sim Parallel Poisson Gamma"
author: "Baoxing Liu"
date: "`r Sys.Date()`"
output:
...
```{r}
k30_m10_df = read_csv("mse_results.csv") %>% 
  mutate(Method = ifelse(Method=="MSE_SRS", "SRS", "Clustered")) %>% 
  filter(k==30)

k5_m60_df = read_csv("mse_results_k5_m60.csv") %>% 
  mutate(Method = ifelse(Method=="MSE_SRS", "SRS", "Clustered"))
df = bind_rows(k30_m10_df,k5_m60_df)

p5 = ggplot(df %>% filter(r==5), aes(x = x, y = sqrt(MSE), color = Method)) +
  geom_line(linewidth=0.25) + geom_point(size = 0.1) +
  facet_grid(k~.,labeller = label_both) +
  theme_minimal(base_size = 13) +
  labs(
    x = "x",
    y = "MSE (sqrt)",
    caption = expression(" "*rho*" = 0.5")
  ) + theme(legend.position = "top")
ggsave("MSE_Poisson_Gamma_bind_n300_r5.pdf",p5)
```
